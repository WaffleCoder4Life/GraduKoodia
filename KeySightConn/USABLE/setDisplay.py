...
def setDisplay(instrument, chan: int, voltageRange_V: float, timeRange_us: float, triggerLevel_V: float) -> str:
    ...
    instrument.write(":TRIGger" + str(chan) + ":MODE EDGE") #NEEDS TESTING
    instrument.write(":TRIGger" + str(chan) + ":COUPling DC") #NEEDS TESTING
    instrument.write(":CHANnel" + str(chan) + ":RANGe " + volt)
    instrument.write(":TIMebase:RANGe " + time + "E-6")
    instrument.write(":TRIGger" + str(chan) + ":LEVel " + str(triggerLevel_V))
