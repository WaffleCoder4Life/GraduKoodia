...
# RUNTIME: measuring t second interval takes t*10000 seconds. t_interval = numberOfDatasets*runTimes*timeRange. E.g. 1 s time interval can be measured with 1000 datasets and 50 runTimes if timeRange is 20E-6. 


settings = {
    "pathNameDate" : "03042024",
    "fileName" : "darkcountTemp",
    "numberOfDatasets" : 5, # Runtime is numberOfDatasets * timeRange * 10 000. Starts lagging with too big datasets.
    "runTimes" : 1, # Runs measurement this many times, used to avoid list overflows. Total measurement interval is
    "biasVoltageRange" : 50,
    "biasVoltage" : 26.7,
    "biasCurrentLimit" : 2.5E-3,
    "timeRange" : 20E-6, # Sets oscilloscope screen width. 20E-6 is good value to use in room temp.
    "peakHeight" : 0.05, # Check before measurement. Example if single peak height is 70 mV, then 0.05 is good value to use.
    "peakDistance" : 400, # Check before measurement with testSingleShot. 400 is good value to use in room temp.
...
def aquireData(settings):
    ...
    while j <= settings["runTimes"]:
        ...
        while i <= settings["numberOfDatasets"]:
            ...
            binaryDataList.append(osc.query_binary_values(":WAVeform:DATA?", datatype = "B")) # Creates a list of lists with binary data read from oscilloscope. Runtime < 0.1 s, 80 000 datapoints
            i += 1
        print(type(binaryDataList[0]))
        print(type(binaryDataList[0][0]))
