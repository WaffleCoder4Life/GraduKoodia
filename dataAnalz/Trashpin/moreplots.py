...
imageTitleUV = "Room temp vs 1.05 K"
saveImageNameUV = f"./dataCollection/{date}/{date}Combo4"
...
def generateFileNameLists(which):
    ...
    Tk().withdraw()
    if which == "uv":
        pathDict[0] = fh.ChooseFilesDifferentFolders(initdir=os.getcwd(), text="Select UV files", filetypes=[('csv files', '*.csv')])
    elif which == "blue":
        pathDict[1] = fh.ChooseFilesDifferentFolders(initdir=os.getcwd(), text="Select Blue files", filetypes=[('csv files', '*.csv')])
    else:
        pathDict[0] = fh.ChooseFilesDifferentFolders(initdir=os.getcwd(), text="Select UV files", filetypes=[('csv files', '*.csv')])
        pathDict[1] = fh.ChooseFilesDifferentFolders(initdir=os.getcwd(), text="Select Blue files", filetypes=[('csv files', '*.csv')])
    return pathDict
...
def main():
    ...
    if len(files) == 1 and whichFiles == "uv":
        ...
        masterData[0] = []
        masterData[1] = []
        colors1 = [cm.Purples(i) for i in np.linspace(0.5, 0.9, len(files[0]))] #dict(uv[tuple(xdata, ydata, str)], blue[tuple(xdata, ydata, str)])
        colors2 = [cm.Blues(i) for i in np.linspace(0.5, 0.9, len(files[1]))]
    ...
    if len(masterData) == 2:
        ...
        for data in masterData:
            for ind in masterData[data]:
                if masterData[data][ind][2] == "NoTPB":
                    plt.scatter(masterData[data][ind][0], masterData[data][ind][1], marker='d', s=10, color=colors1[i], label="Plexiglas")
                elif masterData[data][ind][2] == "noSample":
                    plt.scatter(masterData[data][ind][0], masterData[data][ind][1], marker='d', s=10, color=colors1[i], label="No sample")
                elif masterData[data][ind][2] == "TPB":
                    plt.scatter(masterData[data][ind][0], masterData[data][ind][1], marker='d', s=10, color=colors1[i], label="Plexiglas with TPB")
                i += 1
        plt.xlabel("$I_{\\mathrm{led}}$ / mA", fontsize=13)
        plt.ylabel("$I_{\\mathrm{SiPM}}$ / $\\mu$A", fontsize=13)
        plt.title(imageTitleUV)
