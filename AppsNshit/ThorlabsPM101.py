...
class thorlabsGUI:
    def __init__(self, master):
        ...
        self.butStyle.configure(style='my.TButton', font=('Helvetica', 12), background='black', foreground='royalblue')
        self.butStyle.map('TButton', background=[('active', 'royalblue')], foreground=[('active', 'black')])
        self.enStyle = Style()
        self.enStyle.theme_use('alt')
        self.enStyle.configure("my.TEntry", background='black', foreground='black')
        self.labStyle = Style()
        self.labStyle.configure(style='my.TLabel', font=('Helvetica', 12), background='black', foreground='royalblue')
        self.checkStyle = Style()
        self.checkStyle.theme_use('alt')
        self.checkStyle.configure(style='my.TCheckbutton', background='black', foreground='royalblue')
        ...
        try:
            ...
            self.ax.spines['bottom'].set_color('midnightblue')
            self.ax.spines['top'].set_color('midnightblue') 
            self.ax.spines['right'].set_color('midnightblue')
            self.ax.spines['left'].set_color('midnightblue')
            ...
            if self.rgb:
                ...
                if self.showVar.get() == 0:
                    if self.xdata[-1] > self.xAxisLen:
                        ...
                        self.ax.set_xlabel("Time / s")
                        self.ax.collections.clear()
                        self.sc = self.ax.scatter(self.xdata[self.getIndex():], self.ydata[self.getIndex():], color='midnightblue')
                        self.canvas.draw()
                    else:
                        self.ax.collections.clear()
                        self.sc = self.ax.scatter(self.xdata, self.ydata, color='midnightblue')
                        self.canvas.draw()
                else:
                    self.ax.collections.clear()
                    self.sc = self.ax.scatter(self.xdata, self.ydata, color='midnightblue')
